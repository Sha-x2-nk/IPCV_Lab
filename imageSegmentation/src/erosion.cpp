...
cv::Mat erode(const cv::Mat &img, const int FILTER_SIZE, const std::string &SE_TYPE){
    ...
    // for each loop runs in parallel. 
    out_img.forEach<uchar>([&](uchar &value, const int *position){
                                int r = position[0];
                                int c = position[1];
                                if(r < FILTER_RADIUS || r >= img.rows - FILTER_RADIUS || c < FILTER_RADIUS || c >= img.cols - FILTER_RADIUS)
                                    return;
                                bool is_black = false;
                                if(SE_TYPE == "TRIANGLE"){
